...
const MainMenu = () => {
  ...
  const fetchDataByCategory = async (categoryName) => {
    setData([]);
    try {
      const response = await axiosInstance.get(`/prods/category/${categoryName}`);
  ...
  useEffect(() => {
    ...
    storesItem.forEach((item) => {
      totalQty += parseInt(item.singleData.qty);
      const price = parseFloat(item.singleData.price);
      const qty = parseInt(item.singleData.qty);
      ...
      >
        ...
        <div className='tabs-group lg:w-full xl:w-[500px] 2xl:w-full '>
          <div className='button-group'>
            {
              cats?.map((cate, index) => <button
                key={index} cate
                className={toggleState === index + 1 ? "tab tab-active " : "tab"}
